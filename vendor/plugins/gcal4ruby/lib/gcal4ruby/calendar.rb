...
class Calendar
  ...
  CALENDAR_PUBLIC = "http://www.google.com/calendar/hosted/dowlingmetal.com"
  #The calendar title
  attr_accessor :title
  ...
  def to_iframe(params = {})
    ...
    "<iframe src='#{CALENDAR_PUBLIC}/embed?#{output}' style='#{params[:border]} px solid;' width='#{params[:width]}' height='#{params[:height]}' frameborder='#{params[:border]}' scrolling='no'></iframe>".html_safe
  end
