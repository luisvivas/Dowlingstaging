  ...
  class Grid
    ...
    def initialize( klass, *args, &block )
      ...
      if klass.present? && build_model
        @model = build_columns_later(klass, only, except, col_presets)
      else
        @columns = []
      end
    ...
    def update( options, &block )
      ...
      options.each do |atr, val| 
        raise ArgumentError, "Invalid Gridify update option #{atr.to_s}" unless self.respond_to?("#{atr}=".intern)
        send( "#{atr}=", val )
      end
      yield self if block_given?
    ...
    def build_columns_later(*args)
      self.build_columns_later_args = args
    end
    
    def build_columns_now!
      self.build_columns(*self.build_columns_later_args)
    end
