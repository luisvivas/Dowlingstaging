	...
	class Grid
		...
		attr_accessor :name,								# name of the table (required)
		              ...
		              # pager layer
									...
									:top_pager,							# id of the pager, or true => dom_id+"_pager", false or nil for no pager (nil)
		...
		def columns
		  self.build_columns_now! if self.build_columns_later_args.present?
		  self.build_columns_later_args = nil
		  @columns || []
		end
		def restful
		...
		def title
			...
			else ('&nbsp;' if collapsible || collapsed) #show header with blank title
			end
		end
		...
		def top_pager
		  @top_pager ||= true
		  case @top_pager
      when String then @top_topager
      when true then dom_id+'_toppager'
      end
    end
