...
module Gridify
  class Grid
    ...
    def jqgrid_properties
      ...
      vals['rowNum']            = paging_choices.first
      ...
      if pager
        vals['pager']           = "##{pager}" 
        vals['toppager']        = true if top_pager
    ...
    def jqgrid_javascript( options={} )
      ...
      s = ''
      pagers = []
      pagers.push pager if pager
      pagers.push top_pager if top_pager
      ...
      if arranger_type.include?(:chooser)
        ...
        chooser_code = %Q^ 
        function (){ jQuery('##{dom_id}').jqGrid('columnChooser', {
                          done : function (perm) {
                            if (perm)  {
        ...
        pagers.each do |p|
          s << %Q^ 
          .jqGrid('navButtonAdd','##{p}', #{chooser_opts.to_json.gsub('"chooser_code"', chooser_code)} )^
        end
      end
      ...
      if search_toolbar
        ...
        pagers.each do |p|
          s << %Q^ 
          .jqGrid('navButtonAdd',"##{p}", { caption:"Toggle Search", title:"Toggle Search Toolbar", buttonicon: 'ui-icon-pin-s', onClickButton: function(){ grid[0].toggleToolbar() } })  
          .jqGrid('navButtonAdd',"##{p}", { caption:"Clear", title:"Clear Search", buttonicon: 'ui-icon-refresh', onClickButton: function(){ grid[0].clearToolbar() } }) 
          ^
        end
        s << %Q^
...
class Hash
  ...
  def to_json_with_js
    ActiveSupport::JSON.encode(self).gsub(/\"javascript: ([^"]*)\"/) {|string| string[1..-2].gsub('javascript:','') }
  end
end
