...
describe "Grid finder" do
  ...
  describe "encode" do
    ...
    it "should encode xml" do
      ...
      xml = @grid.encode_records @records, @records.count
...
^
    ...
    it "should encode xml using procs" do
      new_grid :pager => true, :current_page => 1, :data_type => :xml
      @grid.column :unrelated, :proc => Proc.new{|record| record.name[0..0] }
      @grid.column :name, :proc => Proc.new{|record| record.name.upcase }
      xml = @grid.encode_records @records, @records.count
      xml.should == %Q^<?xml version="1.0" encoding="UTF-8"?>
<chickens>
  <chicken>
...
^
    ...
    it "should encode json" do
      ...
      json = @grid.encode_records @records, @records.count
      json.should include('"total_records":2')
      json.should include('"page":1')
      json.should include('"total_pages":1')
      json.should include(
%Q^"chickens":[{"age":10,"created_at":"#{@records[0].created_at.iso8601}","id":1,"name":"Achick"},{"age":20,"created_at":"#{@records[1].created_at.iso8601}","id":2,"name":"Bchick"}]^)
    end
