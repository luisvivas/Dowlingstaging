		...
		return this.each(function(){
			...
			function postIt() {
				...
				if(ret[0] && !rp_ge.processing) {
					...
					var edit_url = (rp_ge.url ? rp_ge.url : $($t).jqGrid('getGridParam','editurl'));
					if($t.p.restful) { edit_url = $t.p.url+"/{id}"; }
					edit_url = (postdata.id == "_empty" ? edit_url.replace("{id}",'') : edit_url.replace("{id}",postdata.id));
